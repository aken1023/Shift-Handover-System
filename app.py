...
def generate_care_report(text):
    try:
        ...
        messages = [
            {
                "role": "system", 
                "content": "你是一個專業的工廠管理人員，擅長整理交接班紀錄。"
            },
            {
                "role": "user",
                "content": f"""
                如果口述內容:{text}與醫療工廠無關，請委婉拒絕回覆，請勿生成任何內容。
                請根據以下口述內容:{text}，整理成一份完整的工廠交接班紀錄，請務必嚴謸依據口述內容回答，不得無中生有，確保數據準確，避免產生不真實或無意義的資訊。

                格式要求：
                請依照以下固定格式撰寫交接班紀錄。若下列問題在口述內容中未提及，請標註「資料不足」，而非自行補充或推測。

                生產進度與數據：當前批次完成數量、運作效率、產量目標達成狀況等。

                設備狀態：重點設備的運作情況、出現故障或異常情況、維修記錄與建議。

                異常事件與處理措施：發生的事故、質量問題、異常現象及其處置情形。

                安全與環保資訊：安全隱患、風險提示、事故報告、環保監控狀況。

                待辦與交接事項：未完成的任務、需要後續跟進的事項和交接注意事項。

                班組人員與工作狀態：人員變動、工時調整、特殊情況說明等。
